...
def addQues(subject):
    ...
    exe_str = "INSERT INTO %s VALUES(%d, '%s', '%s', %s, %s, %s, %s, %s, %s, %s);"\
              % (subject, q_num, in_question, in_type, mcq_op[0], mcq_op[1], mcq_op[2], mcq_op[3], ans_mcq, ans_tf, ans_ow)
...
def delQues(subject):
    ...
    while True:
        in_qNum = int(input(line_break+"Enter the question number to delete or enter '0' to view all the questions:: "))
        if in_qNum == 0:
            sql.execute("USE questions;")
